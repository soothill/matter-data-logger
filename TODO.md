...
- **Issue**: Contains hardcoded `my-super-secret-auth-token` and `adminpassword`
- **Risk**: Secrets in version control, security vulnerability
- **Fix**:
  - Remove hardcoded secrets
  - Use `.env` file with docker-compose
  - Add warning comment about secrets management
  - Document how to generate random tokens

### 2. Update Security-Critical Dependencies
- **Files**: `go.mod`
- **Issue**: Outdated `golang.org/x/crypto v0.21.0` and `golang.org/x/net v0.23.0`
- **Risk**: Known security vulnerabilities
- **Fix**: Run `go get -u golang.org/x/crypto golang.org/x/net` and test

### 3. Add Retry Logic for InfluxDB Writes
- **File**: `storage/influxdb.go`
- **Issue**: Failed writes are logged but not retried
- **Risk**: Data loss during temporary network issues
- **Fix**: Implement exponential backoff retry logic for transient failures

### 4. Enforce TLS for Production InfluxDB
- **File**: `config/config.go`
- **Issue**: No validation warning for HTTP vs HTTPS URLs
- **Risk**: Credentials transmitted in plaintext
- **Fix**: Add validation warning/error for non-HTTPS URLs in production

### 5. Add Vulnerability Scanning to CI/CD
- **Files**: `.github/workflows/ci.yml`
- **Issue**: No automated CVE scanning
- **Risk**: Security vulnerabilities go unnoticed
- **Fix**: Add `govulncheck` step to CI pipeline
...
### 9. Add Input Validation for Power Readings
- **File**: `storage/influxdb.go` (WriteReading method)
- **Issue**: Accepts negative power/voltage/current values
- **Fix**: Add validation:
  ```go
  if reading.Power < 0 || reading.Voltage < 0 || reading.Current < 0 {
      return fmt.Errorf("power readings cannot be negative")
  }
  ```

### 10. Add Comprehensive Configuration Validation
- **File**: `config/config.go`
- **Issue**: Missing validation for URLs, intervals, token formats
- **Fix**: Add validation:
  - URL format parsing
...
### 11. Fix Unbuffered Discovery Channel
- **File**: `discovery/discovery.go:77-109`
- **Issue**: `entries` channel unbuffered, could block zeroconf resolver
- **Fix**: Add buffer: `entries := make(chan *zeroconf.ServiceEntry, 10)`
...
1. Start with all 🔴 CRITICAL items
2. Work through 🟠 HIGH priority items
